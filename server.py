...
class Server(object):
	...
	def get_certfile(self):
		p = os.path.dirname(unicode(__file__, sys.getfilesystemencoding( )))
		if hasattr(sys, "frozen"):
			p = os.path.dirname(unicode(sys.executable, sys.getfilesystemencoding( )))
		return os.path.join(p, 'server.pem')
...
class Client(object):
	...
	def do_join(self, obj):
		...
		self.send(type='channel_joined', channel=self.channel, user_ids=clients)
	...
	def send_to_others(self, **obj):
		try:
			for c in self.server.clients.values():
				if c is not self and c.channel == self.channel:
					c.send(**obj)
		except:
			print 'send to other error'
